...
import requests
from .config import (
    CF_API_BASE, PORTAL_URL, PORTAL_NAME,
...
class TenderScraper:
    ...
    def fetch_releases(self, date_from, date_to):
        ...
        releases = []
        params = {
            'publishedFrom': f"{date_from}T00:00:00Z",
            'publishedTo':   f"{date_to}T23:59:59Z",
            'size':          100,
        }
        url = f"{CF_API_BASE}/Search"
        page = 1
    ...
    def build_direct_url(self, release):
        """Construct the notice URL from the release id.
        CF OCDS release IDs are {uuid}-{sequence} (e.g. 77951900-...-be41-900679).
        The portal only accepts the UUID portion (first 5 hyphen-separated groups).
        """
